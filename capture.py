...
class GameState:
  """
  A GameState specifies the full game gameState, including the food, capsules,
  agent configurations and score changes.

  GameStates are used by the Game object to capture the actual gameState of the game and
  ...
  def generateSuccessor( self, agentIndex, action):
    """
    Returns the successor gameState (a GameState object) after the specified agent takes the action.
    """
  ...
  def __init__( self, prevState = None ):
    """
    Generates a new gameState by copying information from its predecessor.
    """
    if prevState != None: # Initial gameState
  ...
  def initialize( self, layout, numAgents):
    """
    Creates an initial game gameState from a layout array (see layout.py).
    """
  ...
  def process(self, state, game):
    ...
    if state.isOver():
      ...
      if not game.rules.quiet:
        ...
        if blueCount >= foodToWin:#gameState.getRedFood().count() == MIN_FOOD:
          print('The Blue team has returned at least %d of the opponents\' dots.' % foodToWin)
        elif redCount >= foodToWin:#gameState.getBlueFood().count() == MIN_FOOD:
          print('The Red team has returned at least %d of the opponents\' dots.' % foodToWin)
        else:#if gameState.getBlueFood().count() > MIN_FOOD and gameState.getRedFood().count() > MIN_FOOD:
          print('Time is up.')
...
class AgentRules:
  ...
  def applyAction( state, action, agentIndex ):
    """
    Edits the gameState to reflect the results of the action.
    """
  ...
  def consume( position, state, isRed ):
    ...
    if state.data.food[x][y]:
      ...
      #gameState.data.scoreChange += score
      state.data.food = state.data.food.copy()
      state.data.food[x][y] = False
      state.data._foodEaten = position
      #if (isRed and gameState.getBlueFood().count() == MIN_FOOD) or (not isRed and gameState.getRedFood().count() == MIN_FOOD):
  ...
  def dumpFoodFromDeath(state, agentState, agentIndex):
    ...
    #gameState.data.scoreChange += scoreDirection * agentState.numCarrying
